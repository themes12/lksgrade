<?php 
	...
	$server = rand(1,2);
	if($server == 1){
		$serverIP = "61.7.235.50";
	}elseif($server == 2){
		$serverIP = "61.7.235.44";
	}else{
		$serverIP = "61.7.235.44";
	}
	...
	function get($url,$cookieFile, $referer, $params=array()) {	
		...
		curl_setopt($ch, CURLOPT_HTTPHEADER,array('Host: 61.7.235.50'));
	...
	function post($url,$cookieFile, $referer, $fields){
		...
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false); 
		curl_setOpt($ch, CURLOPT_REFERER, $referer);
		curl_setopt($ch, CURLOPT_HTTPHEADER,array('Origin: http://61.7.235.50', 'Host: 61.7.235.50'));
		curl_setopt($ch, CURLOPT_COOKIEJAR, $cookieFile);
		curl_setopt($ch, CURLOPT_COOKIEFILE, $cookieFile); 
		curl_setopt($ch, CURLOPT_VERBOSE, true);
		curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
	...
	$pass=$_SESSION['password'];
  	$curl_scraped_page = get('http://61.7.235.50/semester2551/Default.aspx','cookie.txt','',array('no'=>' '));
	...
	$EventValidation = $html->find("#__EVENTVALIDATION")[0]->attr['value'];
   	...
   	$result =  post('http://61.7.235.50/semester2551/Default.aspx',
			'cookie.txt',
			'http://61.7.235.50/semester2551/Default.aspx', 
			array(
				'__LASTFOCUS'=>'',
